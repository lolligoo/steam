...
class SteamSpider(Spider):
    ...
    start_urls = ['https://store.steampowered.com/search/?ignore_preferences=1&category1=996&page=1']
    timeout_urls = 'timeout_urls.list'
    cur_tag_index_pair = 0

    #con = psycopg2.connect(host="localhost", database="stm", user="stm", password="6B8CoWU6QiY5NZjbcGGuN23ZFxhbk")
    #cur = con.cursor()
    #cur.execute('''DROP TABLE IF EXISTS gameinfo;''')
    #cur.execute('''CREATE TABLE gameinfo
            # (id serial PRIMARY KEY NOT NULL 
            # , title text UNIQUE, url text );''')
    #con.commit()
    #cur.close()
    #con.close()
    # gamematch = open("gamematch.json", "w")
    # gamematch.write("{")
    # gamematch.flush()
    # gamematch.close()
    ...
    def parse(self, response):
        ...
        browse_url_list = [('https://store.steampowered.com/search/?' +
                            'ignore_preferences=1&page={}').format(x)
    ...
    def parse_browse_page(self, response):
        ...
        for game in game_list:
            ...
            if detail_url.find('/bun') > 0:
                print(detail_url)
            if detail_url.find('/sub/') > 0:
                # Skip entries that point to bundles since not tracking them.
                detail_url = detail_url.rsplit('/',1)[0]
            else:
                detail_url = detail_url.rsplit('/',2)[0]
            ...
            title = game.xpath('.//span[@class="title"]/text()')\
                        ...
                        # .replace('\n','')\
                        # .replace('\/','\\/')
            # gamematch = open("gamematch.json", "a")
            # gamematch.write("\"%s\":\"%s\"," % \
            #     (title, detail_url.rsplit('/',2)[0]))
            # gamematch.flush()
            # gamematch.close()
